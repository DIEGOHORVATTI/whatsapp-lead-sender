...
> {
  ...
  override componentDidMount() {
    chrome.storage.local.get(
      ({ message = this.defaultMessage, attachment, delay = 0 }: Omit<Message, 'contact'>) => {
        this.setState({
          message,
          attachment,
          delay,
        ...
        if (attachment?.url && this.fileRef.current) {
          void fetch(attachment.url)
            ...
            .then((blob) => {
              ...
              if (this.fileRef.current) this.fileRef.current.files = dataTransfer.files
            })
        } else {
  ...
  override componentDidUpdate(
    ...
    if (prevState.message !== message) void chrome.storage.local.set({ message })

    if (prevState.delay !== delay) void chrome.storage.local.set({ delay })

    if (prevState.attachment?.url !== attachment?.url) void chrome.storage.local.set({ attachment })
  }
      ...
      >
        <div className="mt-4 mx-4 flex flex-row gap-4">
          <div className="flex flex-col basis-1/2">
            <ControlTextArea value={message} onChange={this.handleMessageChange} />
          </div>
          <div
            className={[
              'px-4',
